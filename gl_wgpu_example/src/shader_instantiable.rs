...
use crate::{Model3DWGpu, ShaderProgram};
...
pub struct ShaderInstantiable<'inst, 'prg> {
    instantiable: &'inst mod3d_base::Instantiable<Model3DWGpu<'prg>>,
    ...
    program: &'inst ShaderProgram,
}

//ip ShaderInstantiable
impl<'inst, 'prg> ShaderInstantiable<'inst, 'prg> {
    //fp new
    /// Create a new [ShaderInstantiable]
    pub fn new(
        context: &mut Model3DWGpu<'prg>,
        program: &'inst ShaderProgram,
        instantiable: &'inst mod3d_base::Instantiable<Model3DWGpu<'prg>>,
    ) -> Result<Self, ()> {
    ...
    pub fn gl_draw(
        &self,
        context: &mut Model3DWGpu<'prg>,
        instance: &mod3d_base::Instance<Model3DWGpu<'prg>>,
    ) {
